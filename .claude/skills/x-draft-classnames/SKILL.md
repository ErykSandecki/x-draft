---
name: x-draft-classnames
description: How to merge a CSS-module base class with an optional caller-supplied className in x-draft — use `cx` from the `classnames` package, never hand-rolled `.filter(Boolean).join(' ')`, and default the prop to `''`. Load before adding a `className` (or similarly-named override) prop to any shared/UI component.
---

# x-draft className Merging
...
it's used at runtime in component code, not a build-time tool). It was added to x-draft
...
x-draft's own convention (a deliberate divergence from x-design, which leaves the prop
...
needing to know anything about that consumer's styling — see [[x-draft-scss-bem]] for how the
consumer then reinforces its override selector's specificity against the shared component's own
base rules.

## Related

[[x-draft-component-structure]] — the general shape of a component file this pattern lives inside.
[[x-draft-scss-bem]] — BEM nesting for the `.module.scss` side of the same components.
[[x-draft-import-order]] — `cx` is a default import from an external package, so it's grouped and

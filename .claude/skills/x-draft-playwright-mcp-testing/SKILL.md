---
name: x-draft-playwright-mcp-testing
description: How to run and demo browser tests for x-draft via the Playwright MCP server (visible Chromium) and via the e2e/ Playwright Test suite. Load before running/writing e2e tests, or when asked to "run/show a test in the browser" or "test the canvas tools".
---

# x-draft Playwright MCP Testing
...
`/design/:id` (`DesignPage`, see [[x-draft-routing]]) — `DesignPage` doesn't validate `:id`, any

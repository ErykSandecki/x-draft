---
name: x-draft-unit-coverage
description: x-draft enforces 100% unit test coverage (branches/functions/lines/statements) via vite.config.ts's coverage.thresholds. Load before saying a change is "done" — run `npm run test:coverage` and add a targeted test for any line the report flags, rather than only running the plain `vitest run` suite.
---

# x-draft Unit Coverage — 100% Enforced
...
[[x-draft-test-act-wrapping]] — completing a gesture in a new test often means adding a
...
[[x-draft-e2e-coverage]] — a different axis: *whether a change needs a permanent Playwright
...
[[x-draft-test-conventions]] — the `describe`/`it` naming and step-comment shape any coverage-gap

---
name: x-draft-test-act-wrapping
...
# x-draft `act()` Wrapping for Raw DOM Event Dispatch
...
DOM node ([[x-draft-function-style]]'s "multiple DOM event handlers" pattern). Their tests mirror
...
`pointerdown` + `pointermove` + `pointerup` together, per [[x-draft-test-conventions]]'s single
...
[[x-draft-test-conventions]] — the `// action` step comment this wraps.
[[x-draft-function-style]] — why these hooks use raw `addEventListener`/`dispatchEvent` instead of

---
name: x-draft-event-handlers-in-hooks
description: JSX event handlers (onClick, onInput, onKeyDown, onBlur, ...) in x-draft components must be extracted into their own hook under the component's hooks/ folder, not defined inline in the component body. Load before adding a new event handler function to a component, or wiring a new on* prop in JSX.
---

# x-draft Event Handlers Live in Hooks
...
[[x-draft-function-style]] — named functions over inline closures inside effects/callbacks; this
skill is the same instinct applied one level up, to the handler itself becoming its own hook.
[[x-draft-module-structure]] — `hooks/` as a subfolder once a component has more than a trivial

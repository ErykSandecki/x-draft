---
name: x-draft-i18n
description: How translations work in x-draft — i18next + react-i18next, flat dot-key JSON, language detection order, and where to add a new string. Load before hardcoding user-facing text in a component, adding a new translation key, or touching translations/.
---
...
Same shape as [[x-draft-theming]]'s `useTheme` (stored choice → system signal → hardcoded default)

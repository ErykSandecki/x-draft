---
name: x-draft-translation-namespace
...
# x-draft Translation Namespace Convention
...
`translations/resources/en.json` / `pl.json` ([[x-draft-i18n]]), never hardcoded in a `constants.ts`
...
reusable feature root — the page is just a thin route wrapper around it ([[x-draft-routing]]).
...
[[x-draft-i18n]] — the flat dot-key JSON convention the resulting keys are added to, and the
`t()`/`useTranslation()` wiring.
[[x-draft-module-structure]] — `constants.ts` already holds other runtime constants; namespace

---
name: x-draft-scss-bem
description: BEM nesting convention for x-draft's .module.scss files — sub-elements of a component go under `&__element` (kebab-case) inside the root class instead of separate flat top-level classes. Load before adding a second class to any .module.scss file, or reviewing one that already has more than one top-level selector.
---

# x-draft SCSS BEM Convention
...
[[x-draft-component-structure]] — the root class still matches the component name exactly; BEM
nesting only governs classes *below* that root.
[[x-draft-import-order]] / [[x-draft-module-structure]] — unrelated axes (import grouping, file
